...
async def safe_click(locator, retries: int = 2, delay: float = 0.6):
    last = None
...
def pop_caption_from_excel(excel_cfg: Dict[str, Any], strategy: str, model_name: str = None) -> str:
    # Special handling for Nathalya - always use the same caption
    if model_name and model_name.lower() == "nathalya":
        single_caption = "wordt nu fan en ontvang een cadeautje 👀🎁\n\n3 MAANDEN = BOOBPLAY\n\n6 MAANDEN = PUSSYPLAY"
        log(f"# [SPECIAL] Using fixed caption for Nathalya")
        return single_caption
    
    # Normal caption logic for other models
...
async def click_create_then_post(page: Page, cfg: Dict[str, Any]):
    ...
    await safe_click(create_btn, retries=cfg["retries"]["clicks"])
    log('# [BTN] Click "Post"')
    post_item = page.locator('a.x9qUeG_linkItem:has(.x9qUeG_name:has-text("Post"))')
    await safe_click(post_item, retries=cfg["retries"]["clicks"])
...
async def click_next_library(page: Page, cfg: Dict[str, Any]):
    ...
    await safe_click(next_btn, retries=cfg["retries"]["clicks"])
...
async def maybe_add_caption(page: Page, cfg: Dict[str, Any], model: Dict[str, Any], post: Dict[str, Any]):
    ...
    caption = pop_caption_from_excel(cap_cfg["excel"], pick_strategy, model["name"])

    log('# [BTN] Click "Edit"')
    edit_btn = page.locator('button._63d-OW_actionButton:has(span._63d-OW_text:text-is("Edit"))')
    await safe_click(edit_btn, retries=cfg["retries"]["clicks"])
    ...
    # 3) Archive used caption (skip for Nathalya since it uses fixed caption)
    if model["name"].lower() != "nathalya":
        archive_used_caption(caption, archive_cfg, model["name"], post.get("name", "post"))
    else:
        log("# [SKIP] Not archiving Nathalya's fixed caption")
...
async def paid_variant_non_fans_pay(page: Page, cfg: Dict[str, Any], price_followers: float):
    ...
    await safe_click(card, retries=cfg["retries"]["clicks"])
...
async def run_rotation_mode(cfg: Dict[str, Any]):
    ...
    while state['current_cycle'] <= max_cycles:
        ...
        if all_done:
            ...
            # Auto-reset after completion for next day
            log("⏳ Waiting 5 minutes before resetting for tomorrow...")
            await sleep_seconds(300, "before daily reset")
            
            # Reset state for next day
            log("🔄 Resetting state for tomorrow's posting cycle...")
            fresh_state = {
                "current_cycle": 1,
                "model_posts": {model["name"]: 0 for model in models},
                "last_run": None,
            ...
            save_state(cfg, fresh_state)
            log("✅ State reset complete! Ready for tomorrow's posting cycle.")
            break
        else:
